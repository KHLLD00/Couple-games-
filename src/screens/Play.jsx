...
import { primeAudio, playAnswerNotification, playRevealNotification } from '../lib/sounds.js'
...
export default function Play({ room, me, onExit, onPlayAgain }) {
  ...
  useEffect(() => {
    let active = true
    setError(''); setRound(null); setReveal(null); setMySubmission(null); setCustomQuestion(null); setMyQuestionSubmitted(false)
  ...
  useEffect(() => subscribeRounds(room.code,(row)=>{
    ...
    if(previous!==null&&row.answer_count>previous){if(row.answer_count===1&&!mySubmission)playAnswerNotification();if(row.answer_count>=2)playRevealNotification()}
  ...
  async function handleCustomQuestion(value){setBusy(true);setError('');try{await submitCustomQuestion(room.code,idx,me,value);setMyQuestionSubmitted(true);const fresh=await fetchRound(room.code,idx);if(fresh)setRound(fresh)}catch(e){setError(e.message||"Couldn't save your question.")}finally{setBusy(false)}}
  ...
  async function handleVerdict(verdict){setBusy(true);setError('');try{await setVerdict(room.code,idx,me,verdict);setReveal(await getReveal(room.code,idx));const updated=await fetchRound(room.code,idx);if(updated)setRound(updated)}catch(e){setError(e.message||"We couldn't save that call. Try again.")}finally{setBusy(false)}}
  async function handleNext(){setBusy(true);setError('');try{if(idx+1>=room.total_rounds)await finishGame(room.code);else await openRound(room.code,idx+1)}catch(e){setError(e.message||"We couldn't move to the next round. Try again.")}finally{setBusy(false)}}

  const exitButton=<div className="mt-6 flex justify-center"><Button variant="ghost" onClick={onExit}>Leave game</Button></div>
  if(room.status==='finished'){const matched=(summary??[]).filter((item)=>item.matched===true).length;const played=summary?.length??0;return <div className="min-h-dvh px-5 pb-8 pt-14"><div className="mx-auto flex min-h-[calc(100dvh-5.5rem)] max-w-lg flex-col"><header className="text-center"><p className="seam-label">And that's a wrap.</p><h1 className="mt-4 text-4xl leading-tight">Same Page</h1><p className="mx-auto mt-4 max-w-xs text-cream/70">You survived every question. Now let's see how often you were actually on the same page.</p></header><section className="my-10 text-center"><p className="font-display text-6xl text-apricot">{matched}/{played}</p><p className="mt-3 text-cream/60">times you agreed</p></section><div className="space-y-3">{(summary??[]).map((item)=><div key={item.idx} className="rounded-2xl border border-cream/10 bg-plum/50 px-4 py-3"><div className="flex items-center justify-between gap-4"><span className="text-sm text-cream/70">Round {item.idx+1} · {MODE_NAMES[item.mode]||'Game'}</span><span className="font-display">{item.matched===true?'Same answer':'Not even close'}</span></div><p className="mt-1 text-sm text-cream/45">{item.prompt}</p></div>)}</div><div className="mt-auto space-y-3 pt-8"><Button onClick={onPlayAgain}>Play Again</Button>{exitButton}</div></div></div>}
  ...
  return <div className="min-h-dvh px-5 pb-8 pt-14"><div className="mx-auto flex min-h-[calc(100dvh-5.5rem)] max-w-lg flex-col"><div className="mb-3 text-center"><span className="seam-label">{MODE_NAMES[round.mode]||'Round'} · Round {round.idx+1}</span></div><Question round={activeRound} mySubmission={mySubmission} myQuestionSubmitted={myQuestionSubmitted} onSubmit={handleSubmit} onSubmitCustomQuestion={handleCustomQuestion} busy={busy}/><div className="mt-auto">{exitButton}</div>{error&&<p className="fixed bottom-4 left-5 right-5 text-center text-sm text-cherry">{error}</p>}</div></div>
}
