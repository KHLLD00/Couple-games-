...
export default function App() {
  ...
  useEffect(() => { let active=true; async function initialisePlayer(){ try { const {data:{session}}=await supabase.auth.getSession(); let user=session?.user; if(!user){const {data,error:authError}=await supabase.auth.signInAnonymously(); if(authError) throw authError; user=data.user} if(!user) throw new Error('Could not create a player session.'); if(active) setMe(user.id) } catch(e){if(active)setError(e.message||'Could not create a player session.')} finally{if(active)setBusy(false)} } initialisePlayer(); return()=>{active=false} },[])
  useEffect(() => { if(!room) return; return subscribeRoom(room.code,(updated)=>{ if(!updated){setRoom(null);setSetup(false);setError('Your partner left. This game has ended.');if(window.location.pathname!=='/')window.history.replaceState({},'','/');return} setRoom(updated) }) },[room?.code])
  ...
  async function handlePlayAgain(){setBusy(true);setError('');try{setRoom(await resetGame(room.code))}catch(e){setError(e.message||'Could not start another game.')}finally{setBusy(false)}}
  function handleExit(){const code=room?.code;setRoom(null);setSetup(false);setError('');if(window.location.pathname!=='/')window.history.replaceState({},'','/');if(code&&me)leaveRoom(code,me).catch(()=>{})}
  ...
  if(room.status==='lobby')return <Lobby room={room} isHost={room.host_id===me} onStart={handleStart} onSetup={()=>setSetup(true)} onExit={handleExit} busy={busy}/>
