...
export default function App() {
  ...
  async function handleCreate(name) {
    ...
    try { setRoom(await createRoom(me, name)) }
    catch (e) { setError(e.message || 'Could not create the room.') }
    finally { setBusy(false) }
  }

  async function handleJoin(code, name) {
    setBusy(true); setError('')
    try { setRoom(await joinRoom(code, me, name)); window.history.replaceState({}, '', '/') }
    catch (e) { setError(e.message || 'Could not join the room.') }
    finally { setBusy(false) }
  }

  async function handleStart() {
    setBusy(true); setError('')
    try { await openRound(room.code, 0) }
    catch (e) { setError(e.message || 'Could not start the game.') }
    finally { setBusy(false) }
  }
